...
class DynamicFlowHelper:
    ...
    def format_field_data(self, field, format_sub_fields=None, get_visibility_conditions=None):
        """
        Helper function to format a single field with all validations and attributes,
        removing irrelevant validators based on the field type.
        """
        ...
        field_data = {
            ...
            "allowed_lookups": [
                {"name": lookup.name, "id": lookup.id, "code": lookup.code, "icon": lookup.icon}
        ...
        # Add relevant validations based on field type
    ...
    def format_category(self, category, fields):
        category_fields = [
            self.format_field_data(field, self.format_sub_fields, self.get_visibility_conditions)
    ...
    def format_page(self, page, fields):
        ...
        return {
            ...
            "page_id": page.id,
            "categories": [self.format_category(category, fields) for category in page_categories],
        }

    def format_sub_fields(self, parent_field):
        """
        Recursively format sub-fields for a given parent field.
        """
        sub_fields_data = []
    ...
    def get_visibility_conditions(self, field):
        """
        Retrieve visibility conditions for a given field.
        """
