...
class DynamicFlowValidator:
    ...
    def fetch_lookup_choices(self, lookup_field: str) -> List[int]:
        ...
        try:
            parent_lookup = Lookup.objects.get(id=int(lookup_field), type=Lookup.LookupTypeChoices.LOOKUP)
    ...
    def _validate_json(self, value: Any, field_info: Dict[str, Any]) -> List[str]:
        """
        Validate JSON fields with nested sub-fields and constraints on the number of objects.
        """
        errors = []
    ...
    def _validate_nested_fields(self, data: Dict[str, Any], sub_fields: List[Dict[str, Any]], context: str) -> List[str]:
        ...
        for sub_field_info in sub_fields:
            ...
            if nested_sub_fields and sub_field_value is not None:
                ...
                # Validate constraints for the JSON list itself
                max_items = sub_field_info.get("max_length")
                min_items = sub_field_info.get("min_length")

                if max_items is not None and len(sub_field_value) > max_items:
                    errors.append(f"{context}: Sub-field '{sub_field_name}' exceeds the maximum allowed number of items ({max_items}).")

                if min_items is not None and len(sub_field_value) < min_items:
                    errors.append(f"{context}: Sub-field '{sub_field_name}' has fewer items than the minimum required ({min_items}).")
