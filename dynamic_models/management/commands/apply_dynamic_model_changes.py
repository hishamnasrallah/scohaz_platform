...
from dynamic_models.utils.utils import apply_schema_changes_for_model, reload_database_schema
import logging
    ...
    def handle(self, *args, **options):
        ...
        for dynamic_model in queryset:
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Schema changes applied for model '{dynamic_model.name}'."
                    )
                )
            except Exception as e:
                logger.error(
                    f"Error applying schema changes for model '{dynamic_model.name}': {e}"
                )
                self.stderr.write(
                    self.style.ERROR(
                        f"Error applying schema changes for model '{dynamic_model.name}': {e}"
