    ...
    def generate_fields(self):
        ...
        for field in self.model._meta.get_fields():
            ...
            if isinstance(field, models.CharField):
                fields[field.name] = forms.CharField(label=field.verbose_name, max_length=field.max_length, required=not field.blank)
            elif isinstance(field, models.IntegerField):
                fields[field.name] = forms.IntegerField(label=field.verbose_name, required=not field.blank)
            elif isinstance(field, models.BooleanField):
                fields[field.name] = forms.BooleanField(label=field.verbose_name, required=not field.blank)
            elif isinstance(field, models.ForeignKey):
                related_model = apps.get_model('{app_name}', field.related_model.__name__)
                fields[field.name] = forms.ModelChoiceField(queryset=related_model.objects.all(), required=not field.blank)

        return fields

    def get_validation_rules(self):
        ValidationRule = apps.get_model('{app_name}', 'ValidationRule')
        return ValidationRule.objects.filter(model_name=self.model.__name__)
        ...
        for rule in self.validation_rules:
            ...
            if rule.user_roles:
                ...
                if not user_groups.intersection(required_roles):
                    raise ValidationError(f"User does not have the correct role to validate {{field_name}}")

            # Apply condition-based validation
            conditions = rule.condition_logic or []
            if conditions:
                condition_evaluator = ConditionEvaluator(self.cleaned_data)
            ...
            if not validator:
                raise ValidationError(f"Unknown validator type: {{rule.rule_type}}")

            # Apply the validator to the field
            validator(field_value, *rule.params, error_message=rule.error_message, instance=self)

        return cleaned_data
"""

        # Write the dynamic form builder code to a forms.py file
    ...
    def generate_admin_file(self, app_path, models, app_name):
        ...
        admin_code += f"from {app_name}.mixins import AdminUserContextMixin\n"
        # admin_code += "from django.db import models\n\n"
        admin_code += f"from {app_name}.utils.validators import VALIDATOR_REGISTRY\n"
        admin_code += f"from {app_name}.forms import DynamicFormBuilder\n\n"
...
"""
    ...
    def get_form(self, request, obj=None, **kwargs):
        \"\"\"
        Override get_form to dynamically generate the form.
        \"\"\"
        # Dynamically generate the form fields based on the model
        form_class = DynamicFormBuilder(model_name=self.model.__name__, user=request.user)
        return form_class
"""
